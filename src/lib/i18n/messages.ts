/**
 * RENEW — UI message catalogs.
 *
 * English is the complete base and the guaranteed fallback: any key missing
 * from another language falls back to English, so the product is always fully
 * usable while catalogs are expanded toward every supported language. Keys are
 * dotted namespaces ("settings.region.title"); values may contain {named}
 * placeholders substituted by translate().
 */

export type MessageKey = keyof typeof EN;
export type Catalog = Partial<Record<MessageKey, string>>;

/** The English base — the single source of truth for available keys. */
export const EN = {
  "common.save": "Save",
  "common.cancel": "Cancel",
  "common.edit": "Edit",
  "common.delete": "Delete",
  "common.add": "Add",
  "common.done": "Done",
  "common.close": "Close",
  "common.loading": "Loading…",
  "common.retry": "Try again",
  "common.saving": "Saving…",
  "common.search": "Search",
  "common.date": "Date",
  "common.error": "Something went wrong",
  "common.category": "Category",
  "search.placeholder": "Search transactions, bills, goals…",
  "search.empty": "No matches",
  "search.hint": "Search across your whole account",

  "install.cta": "Install Renew",
  "install.ios.title": "Install Renew",
  "install.ios.body": "Tap the Share button, then choose “Add to Home Screen” to install Renew.",
  "install.ios.gotit": "Got it",

  "date.today": "Today",
  "date.tomorrow": "Tomorrow",
  "date.yesterday": "Yesterday",

  "notif.reminder.due": "Reminder due today",
  "notif.reminder.overdue": "Reminder overdue",
  "notif.task.due": "Task due today",
  "notif.task.overdue": "Task overdue",
  "notif.payment.due": "Bill due soon",
  "notif.payment.overdue": "Bill overdue",
  "notif.document.due": "Document expiring soon",
  "notif.document.overdue": "Document expired",
  "notif.budget.warn.title": "Budget almost reached",
  "notif.budget.warn.body": "You've used {percent}% of your {category} budget.",
  "notif.budget.over.title": "Budget exceeded",
  "notif.budget.over.body": "You've gone over your {category} budget this month.",
  "notif.savings.reached.title": "Savings goal reached",
  "notif.savings.reached.body": "You've reached your goal: {name}. 🎉",

  "nav.dashboard": "Overview",
  "nav.income": "Income",
  "nav.transactions": "Transactions",
  "nav.budget": "Budget",
  "nav.savings": "Savings",
  "nav.investments": "Investments",
  "nav.payments": "Bills",
  "nav.calendar": "Calendar",
  "nav.analytics": "Analytics",
  "nav.settings": "Settings",
  "nav.accounts": "Accounts",
  "nav.subscriptions": "Subscriptions",
  "nav.more": "More",

  "accounts.new": "New account",
  "accounts.edit": "Edit account",
  "accounts.name": "Account name",
  "accounts.type": "Type",
  "accounts.institution": "Institution",
  "accounts.opening": "Opening balance",
  "accounts.note": "Note",
  "accounts.total": "Total balance",
  "accounts.archived": "Archived",
  "accounts.created": "Account created",
  "accounts.updated": "Account updated",
  "accounts.deleted": "Account deleted",
  "accounts.empty.title": "No accounts yet",
  "accounts.empty.body": "Add a cash, bank, card or savings account — Renew keeps each balance up to date from your activity.",
  "accounts.delete.title": "Delete this account?",
  "accounts.delete.body": "This removes the account. Your transactions are kept — they'll simply show as unassigned. Prefer Archive if you want to keep it out of the way.",
  "accounts.recentTransfers": "Recent transfers",
  "accounts.transfer": "Transfer",
  "accounts.transfer.hint": "Move money between two of your accounts in the same currency.",
  "accounts.transfer.from": "From",
  "accounts.transfer.to": "To",
  "accounts.transfer.amount": "Amount",
  "accounts.transfer.done": "Transfer recorded",

  "subs.new": "New subscription",
  "subs.edit": "Edit subscription",
  "subs.name": "Service name",
  "subs.price": "Price",
  "subs.cycle": "Billing cycle",
  "subs.next": "Next billing",
  "subs.account": "Billed to",
  "subs.monthly": "Monthly",
  "subs.annual": "Yearly",
  "subs.active": "Active",
  "subs.cancelled": "Cancelled",
  "subs.cycle.weekly": "Weekly",
  "subs.cycle.monthly": "Monthly",
  "subs.cycle.quarterly": "Quarterly",
  "subs.cycle.yearly": "Yearly",
  "subs.cancel": "Cancel",
  "subs.reactivate": "Reactivate",
  "subs.created": "Subscription added",
  "subs.updated": "Subscription updated",
  "subs.empty.title": "No subscriptions yet",
  "subs.empty.body": "Track recurring services like streaming, software or memberships — see what they cost per month and year.",
  "notif.subscription.due": "Subscription renews soon",

  "settings.title": "Settings",
  "settings.subtitle": "Make Renew feel like yours.",
  "settings.region.title": "Region & Language",
  "settings.region.language": "Language",
  "settings.region.region": "Country / region",
  "settings.region.currency": "Currency",
  "settings.region.timezone": "Timezone",
  "settings.region.weekStart": "Week starts on",
  "settings.region.weekStart.sunday": "Sunday",
  "settings.region.weekStart.monday": "Monday",
  "settings.region.hint":
    "Auto-detected from your device. Change anything — your choices are saved to your account.",
  "settings.region.saved": "Region & language updated",

  "settings.notify.browser": "Browser notifications",
  "settings.notify.browser.hint": "Get an OS notification when something needs attention while Renew is open.",
  "settings.notify.browser.enable": "Enable",
  "settings.notify.browser.on": "On",
  "settings.notify.browser.blocked": "Blocked — allow notifications in your browser settings",
  "settings.notify.browser.unsupported": "Not supported on this browser",

  "settings.data.title": "Data",
  "settings.data.hint": "Download a copy of your financial data. It stays on your device — Renew never sells or shares it.",
  "settings.data.exportJson": "Export all (JSON)",
  "settings.data.exportCsv": "Export transactions (CSV)",
  "settings.data.exported": "Export ready — check your downloads",
  "settings.data.empty": "Nothing to export yet",
  "settings.data.import": "Import transactions (CSV)",
  "settings.data.import.title": "Import transactions",
  "settings.data.import.summary": "{valid} ready to import · {duplicates} duplicates skipped · {invalid} invalid rows",
  "settings.data.import.confirm": "Import {valid}",
  "settings.data.import.none": "No new transactions found in that file",
  "settings.data.import.done": "Imported {count} transactions",
  "settings.data.import.failed": "Couldn't read that file",
} as const;

/** Non-English catalogs. Partial by design; English fills every gap. */
const CATALOGS: Record<string, Catalog> = {
  es: {
    "common.save": "Guardar",
    "common.cancel": "Cancelar",
    "common.edit": "Editar",
    "common.delete": "Eliminar",
    "common.add": "Añadir",
    "common.done": "Hecho",
    "common.close": "Cerrar",
    "common.loading": "Cargando…",
    "common.retry": "Reintentar",
    "common.saving": "Guardando…",
    "common.search": "Buscar",
    "nav.dashboard": "Resumen",
    "nav.transactions": "Transacciones",
    "nav.budget": "Presupuesto",
    "nav.savings": "Ahorros",
    "nav.investments": "Inversiones",
    "nav.payments": "Facturas",
    "nav.calendar": "Calendario",
    "nav.analytics": "Análisis",
    "nav.settings": "Ajustes",
    "nav.more": "Más",
    "date.today": "Hoy",
    "date.tomorrow": "Mañana",
    "date.yesterday": "Ayer",
    "notif.reminder.due": "Recordatorio para hoy",
    "notif.reminder.overdue": "Recordatorio vencido",
    "notif.payment.due": "Factura por vencer",
    "notif.payment.overdue": "Factura vencida",
    "notif.budget.warn.title": "Presupuesto casi alcanzado",
    "notif.budget.warn.body": "Has usado el {percent}% de tu presupuesto de {category}.",
    "notif.budget.over.title": "Presupuesto superado",
    "notif.budget.over.body": "Has superado tu presupuesto de {category} este mes.",
    "notif.savings.reached.title": "Meta de ahorro alcanzada",
    "notif.savings.reached.body": "Has alcanzado tu meta: {name}. 🎉",
    "settings.region.title": "Región e idioma",
    "settings.region.language": "Idioma",
    "settings.region.region": "País / región",
    "settings.region.currency": "Moneda",
    "settings.region.timezone": "Zona horaria",
    "settings.region.weekStart": "La semana empieza en",
    "settings.region.weekStart.sunday": "Domingo",
    "settings.region.weekStart.monday": "Lunes",
    "common.date": "Fecha",
    "common.error": "Algo salió mal",
    "common.category": "Categoría",
    "search.placeholder": "Buscar transacciones, facturas, metas…",
    "search.empty": "Sin resultados",
    "search.hint": "Busca en toda tu cuenta",
    "install.cta": "Instalar Renew",
    "nav.accounts": "Cuentas",
    "nav.subscriptions": "Suscripciones",
    "accounts.new": "Nueva cuenta",
    "accounts.edit": "Editar cuenta",
    "accounts.name": "Nombre de la cuenta",
    "accounts.type": "Tipo",
    "accounts.institution": "Institución",
    "accounts.opening": "Saldo inicial",
    "accounts.note": "Nota",
    "accounts.total": "Saldo total",
    "accounts.archived": "Archivadas",
    "accounts.created": "Cuenta creada",
    "accounts.updated": "Cuenta actualizada",
    "accounts.deleted": "Cuenta eliminada",
    "accounts.recentTransfers": "Transferencias recientes",
    "accounts.transfer": "Transferir",
    "accounts.transfer.from": "Desde",
    "accounts.transfer.to": "Hacia",
    "accounts.transfer.amount": "Importe",
    "accounts.transfer.done": "Transferencia registrada",
    "subs.new": "Nueva suscripción",
    "subs.edit": "Editar suscripción",
    "subs.name": "Servicio",
    "subs.price": "Precio",
    "subs.cycle": "Ciclo de facturación",
    "subs.next": "Próximo cobro",
    "subs.account": "Se cobra a",
    "subs.monthly": "Mensual",
    "subs.annual": "Anual",
    "subs.cancelled": "Canceladas",
    "subs.cycle.weekly": "Semanal",
    "subs.cycle.monthly": "Mensual",
    "subs.cycle.quarterly": "Trimestral",
    "subs.cycle.yearly": "Anual",
    "notif.subscription.due": "La suscripción se renueva pronto",
  },
  fr: {
    "common.save": "Enregistrer",
    "common.cancel": "Annuler",
    "common.edit": "Modifier",
    "common.delete": "Supprimer",
    "common.add": "Ajouter",
    "common.loading": "Chargement…",
    "nav.dashboard": "Aperçu",
    "nav.transactions": "Transactions",
    "nav.budget": "Budget",
    "nav.savings": "Épargne",
    "nav.investments": "Investissements",
    "nav.payments": "Factures",
    "nav.calendar": "Calendrier",
    "nav.analytics": "Analyses",
    "nav.settings": "Paramètres",
    "nav.more": "Plus",
    "date.today": "Aujourd’hui",
    "date.tomorrow": "Demain",
    "date.yesterday": "Hier",
    "settings.region.weekStart.sunday": "Dimanche",
    "settings.region.weekStart.monday": "Lundi",
    "settings.region.title": "Région et langue",
    "settings.region.currency": "Devise",
    "settings.region.timezone": "Fuseau horaire",
    "common.date": "Date",
    "common.error": "Une erreur s’est produite",
    "common.category": "Catégorie",
    "common.search": "Rechercher",
    "search.placeholder": "Rechercher transactions, factures, objectifs…",
    "search.empty": "Aucun résultat",
    "search.hint": "Cherchez dans tout votre compte",
    "install.cta": "Installer Renew",
    "nav.accounts": "Comptes",
    "nav.subscriptions": "Abonnements",
    "accounts.new": "Nouveau compte",
    "accounts.edit": "Modifier le compte",
    "accounts.name": "Nom du compte",
    "accounts.type": "Type",
    "accounts.institution": "Établissement",
    "accounts.opening": "Solde d’ouverture",
    "accounts.note": "Note",
    "accounts.total": "Solde total",
    "accounts.archived": "Archivés",
    "accounts.created": "Compte créé",
    "accounts.updated": "Compte mis à jour",
    "accounts.deleted": "Compte supprimé",
    "accounts.recentTransfers": "Virements récents",
    "accounts.transfer": "Virement",
    "accounts.transfer.from": "De",
    "accounts.transfer.to": "Vers",
    "accounts.transfer.amount": "Montant",
    "accounts.transfer.done": "Virement enregistré",
    "subs.new": "Nouvel abonnement",
    "subs.edit": "Modifier l’abonnement",
    "subs.name": "Service",
    "subs.price": "Prix",
    "subs.cycle": "Cycle de facturation",
    "subs.next": "Prochain paiement",
    "subs.account": "Facturé sur",
    "subs.monthly": "Mensuel",
    "subs.annual": "Annuel",
    "subs.cancelled": "Annulés",
    "subs.cycle.weekly": "Hebdomadaire",
    "subs.cycle.monthly": "Mensuel",
    "subs.cycle.quarterly": "Trimestriel",
    "subs.cycle.yearly": "Annuel",
    "notif.subscription.due": "L’abonnement se renouvelle bientôt",
  },
  de: {
    "common.save": "Speichern",
    "common.cancel": "Abbrechen",
    "common.delete": "Löschen",
    "nav.dashboard": "Übersicht",
    "nav.transactions": "Transaktionen",
    "nav.budget": "Budget",
    "nav.savings": "Ersparnisse",
    "nav.investments": "Investitionen",
    "nav.payments": "Rechnungen",
    "nav.settings": "Einstellungen",
    "nav.more": "Mehr",
    "date.today": "Heute",
    "date.tomorrow": "Morgen",
    "date.yesterday": "Gestern",
    "settings.region.title": "Region & Sprache",
    "settings.region.currency": "Währung",
    "settings.region.weekStart.sunday": "Sonntag",
    "settings.region.weekStart.monday": "Montag",
    "common.date": "Datum",
    "common.error": "Etwas ist schiefgelaufen",
    "common.category": "Kategorie",
    "common.search": "Suchen",
    "search.placeholder": "Transaktionen, Rechnungen, Ziele suchen…",
    "search.empty": "Keine Treffer",
    "search.hint": "Durchsuche dein ganzes Konto",
    "install.cta": "Renew installieren",
    "nav.accounts": "Konten",
    "nav.subscriptions": "Abos",
    "nav.calendar": "Kalender",
    "nav.analytics": "Analysen",
    "accounts.new": "Neues Konto",
    "accounts.edit": "Konto bearbeiten",
    "accounts.name": "Kontoname",
    "accounts.type": "Typ",
    "accounts.institution": "Institut",
    "accounts.opening": "Anfangssaldo",
    "accounts.note": "Notiz",
    "accounts.total": "Gesamtsaldo",
    "accounts.archived": "Archiviert",
    "accounts.created": "Konto erstellt",
    "accounts.updated": "Konto aktualisiert",
    "accounts.deleted": "Konto gelöscht",
    "accounts.recentTransfers": "Letzte Überweisungen",
    "accounts.transfer": "Überweisen",
    "accounts.transfer.from": "Von",
    "accounts.transfer.to": "Nach",
    "accounts.transfer.amount": "Betrag",
    "accounts.transfer.done": "Überweisung erfasst",
    "subs.new": "Neues Abo",
    "subs.edit": "Abo bearbeiten",
    "subs.name": "Dienst",
    "subs.price": "Preis",
    "subs.cycle": "Abrechnungszeitraum",
    "subs.next": "Nächste Abrechnung",
    "subs.account": "Abgerechnet über",
    "subs.monthly": "Monatlich",
    "subs.annual": "Jährlich",
    "subs.cancelled": "Gekündigt",
    "subs.cycle.weekly": "Wöchentlich",
    "subs.cycle.monthly": "Monatlich",
    "subs.cycle.quarterly": "Vierteljährlich",
    "subs.cycle.yearly": "Jährlich",
    "notif.subscription.due": "Abo wird bald verlängert",
  },
  hi: {
    "common.save": "सहेजें",
    "common.cancel": "रद्द करें",
    "nav.dashboard": "अवलोकन",
    "nav.transactions": "लेन-देन",
    "nav.budget": "बजट",
    "nav.savings": "बचत",
    "nav.investments": "निवेश",
    "nav.payments": "बिल",
    "nav.settings": "सेटिंग्स",
    "nav.more": "और",
    "date.today": "आज",
    "date.tomorrow": "कल",
    "date.yesterday": "बीता कल",
    "settings.region.title": "क्षेत्र और भाषा",
  },
  ar: {
    "common.save": "حفظ",
    "common.cancel": "إلغاء",
    "nav.dashboard": "نظرة عامة",
    "nav.transactions": "المعاملات",
    "nav.budget": "الميزانية",
    "nav.savings": "المدخرات",
    "nav.investments": "الاستثمارات",
    "nav.payments": "الفواتير",
    "nav.settings": "الإعدادات",
    "nav.more": "المزيد",
    "date.today": "اليوم",
    "date.tomorrow": "غدًا",
    "date.yesterday": "أمس",
    "settings.region.title": "المنطقة واللغة",
  },
};

function interpolate(template: string, vars?: Record<string, string | number>) {
  if (!vars) return template;
  return template.replace(/\{(\w+)\}/g, (_, k: string) =>
    k in vars ? String(vars[k]) : `{${k}}`,
  );
}

/**
 * Resolve a message key for the given language, falling back to English.
 * `vars` substitutes {named} placeholders.
 */
export function translate(
  language: string,
  key: MessageKey,
  vars?: Record<string, string | number>,
): string {
  const base = language.split("-")[0] ?? language;
  const localized = CATALOGS[base]?.[key];
  return interpolate(localized ?? EN[key] ?? key, vars);
}
