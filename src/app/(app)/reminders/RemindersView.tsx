...
export function RemindersView() {
  ...
  const active = useMemo(
    () => data.filter((r) => !r.completed).sort((a, b) => a.dueAt - b.dueAt),
    [data],
  );
  ...
  async function onComplete(reminder: Reminder) {
    ...
    try {
      ...
      if (res.renewed && res.nextDue) {
        toast({
          title: "Renewed",
          description: `Next: ${dueLabel(res.nextDue, reminder.hasTime)}`,
          variant: "success",
        });
      }
    } catch {
  ...
  async function onDelete(reminder: Reminder) {
    ...
    try {
      ...
      toast({
        ...
        action: {
          label: "Undo",
          onClick: () => restoreReminder(uid, reminder).catch(() => {}),
        },
      });
    } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        title="Reminders"
        subtitle="Everything that shouldn't slip — in one calm list."
        action={
      ...
      <form onSubmit={onQuickAdd} className="mb-5">
        <div className="glass flex items-center gap-2 p-2 pl-4">
          ...
          <input
            ...
            className="h-9 flex-1 bg-transparent text-sm text-[var(--text-strong)] placeholder:text-[var(--text-muted)] focus:outline-none"
          />
          {quick.trim() && (
            <button
              type="submit"
              disabled={adding}
              className="flex items-center gap-1.5 rounded-full bg-gradient-to-b from-gold-200 to-gold-400 px-3 py-1.5 text-xs font-medium text-[var(--text-onGold)] disabled:opacity-60"
            >
        ...
        {(["active", "completed"] as Tab[]).map((t) => (
          ...
          >
            ...
            {t === "active" && active.length > 0 && (
              <span className="ml-1.5 tabular-nums">{active.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {isEmpty ? (
        <GlassCard padded>
          <EmptyState
            icon={Bell}
            title={tab === "active" ? "No active reminders" : "Nothing completed yet"}
            description={
              tab === "active"
                ? "Add your first reminder above, or create one with full details."
                : "Completed reminders will gather here."
            }
            action={
              tab === "active" ? (
                <AnimatedButton onClick={openCreate}>
  ...
  function renderRow(reminder: Reminder) {
    return (
      <ReminderRow
        ...
        onReopen={() => uid && reopenReminder(uid, reminder.id)}
...
function Section({
  ...
  title: string;
  items: Reminder[];
  render: (r: Reminder) => React.ReactNode;
  tone?: "danger";
}) {
    ...
    <div>
      <h2
        className={cn(
          ...
          tone === "danger" ? "text-rose-500" : "text-[var(--text-muted)]",
        )}
      >
        {title}
        <span className="ml-2 tabular-nums opacity-70">{items.length}</span>
      </h2>
      <div className="flex flex-col gap-2">
        <AnimatePresence initial={false}>{items.map(render)}</AnimatePresence>
      </div>
    </div>
  );
}
