            ...
            {days.map((day) => {
              ...
              return (
                <button key={day.toISOString()} type="button" onClick={() => setSelected(day)} aria-label={dayAria.format(day)} aria-pressed={isSel}
                  className={cn("relative flex aspect-square flex-col items-center justify-start gap-0.5 rounded-xl p-0.5 text-sm transition-colors sm:p-1.5", inMonth ? "text-[var(--text-body)]" : "text-[var(--text-muted)]/50", isSel ? "bg-[var(--glass-bg-strong)] shadow-[inset_0_1px_0_var(--glass-edge)]" : "hover:bg-[var(--glass-bg-soft)]")}>
