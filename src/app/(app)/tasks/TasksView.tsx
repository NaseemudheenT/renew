...
export function TasksView() {
  ...
  const active = useMemo(
    () =>
      data
        ...
        .sort((a, b) => {
          // Dated tasks first (earliest), then by most recent.
  ...
  async function onToggle(task: Task) {
    if (!uid) return;
    await setTaskCompleted(uid, task.id, !task.completed).catch(() =>
  ...
  async function onDelete(task: Task) {
    ...
    try {
      ...
      toast({
        ...
        action: {
          label: "Undo",
          onClick: () => restoreTask(uid, task).catch(() => {}),
        },
      });
    } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        title="Tasks"
        subtitle="Quick to capture, satisfying to finish."
        action={
      ...
      {isEmpty ? (
        <GlassCard padded>
          <EmptyState
            icon={ListTodo}
            title={tab === "active" ? "No active tasks" : "Nothing completed yet"}
            description={
              tab === "active"
                ? "Capture your first task above — a title is all you need."
                : "Finished tasks will gather here."
            }
            action={
              tab === "active" ? (
                <AnimatedButton onClick={openCreate}>
