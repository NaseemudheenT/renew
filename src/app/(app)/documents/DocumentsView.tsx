...
export function DocumentsView() {
  ...
  // Keep the open doc in sync with realtime updates.
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader
        title="Documents"
        subtitle="One secure home for the papers that matter."
      />
      ...
      {!loading && data.length === 0 ? (
        <GlassCard padded>
          <EmptyState
            ...
            description="Upload passports, insurance policies, licenses — anything you want kept safe and easy to find. Drag a file above to begin."
          />
        </GlassCard>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          <AnimatePresence>
            {filtered.map((doc) => (
              <DocumentCard key={doc.id} doc={doc} onOpen={() => setOpenDoc(doc)} />
            ))}
          </AnimatePresence>
        </div>
      )}
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "border-[var(--field-border)] bg-[var(--field-bg)] text-[var(--text-muted)] hover:text-[var(--text-strong)]",
      )}
    >
      {label}
    </button>
