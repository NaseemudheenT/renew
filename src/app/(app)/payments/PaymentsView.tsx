...
export function PaymentsView() {
  ...
  const overdue = useMemo(
    () => upcoming.filter((p) => isOverdue(p.dueAt)),
    [upcoming],
  );
  const notOverdue = useMemo(
    () => upcoming.filter((p) => !isOverdue(p.dueAt)),
    [upcoming],
  );
  ...
  async function onPay(payment: Payment) {
    ...
    if (res.rolled && res.nextDue) {
      toast({
        title: "Marked paid",
        description: `Next due ${dueLabel(res.nextDue)}`,
        variant: "success",
      });
    } else {
  ...
  async function onDelete(payment: Payment) {
    ...
    try {
      ...
      toast({
        ...
        action: {
          label: "Undo",
          onClick: () => restorePayment(uid, payment).catch(() => {}),
        },
      });
    } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        title="Payments"
        subtitle="Keep on top of what's due — never miss an important one."
        action={
          ...
          <GlassCard className="p-4">
            <div
              className={cn(
                ...
                overdue.length ? "text-rose-500" : "text-[var(--text-strong)]",
              )}
            >
              {overdue.length}
            </div>
      ...
      {isEmpty ? (
        <GlassCard padded>
          <EmptyState
            ...
            description="Add rent, insurance, subscriptions or any bill you want to keep an eye on — and mark them paid as you go."
            action={
              <AnimatedButton onClick={openCreate}>
                <Plus className="size-4" />
                New payment
        ...
        <GlassCard padded>
          <EmptyState
            compact
            icon={Wallet}
            title={tab === "upcoming" ? "Nothing due — you're clear" : "No paid payments yet"}
          />
        </GlassCard>
      ) : tab === "upcoming" ? (
        ...
        <div className="flex flex-col gap-2">
          <AnimatePresence initial={false}>{paid.map(renderRow)}</AnimatePresence>
        </div>
      )}
  ...
  function renderRow(payment: Payment) {
    return (
      <PaymentRow
        key={payment.id}
        payment={payment}
        onPay={() => onPay(payment)}
        onUnpay={() => uid && markUnpaid(uid, payment.id)}
