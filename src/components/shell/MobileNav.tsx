...
export function MobileNav() {
  ...
  const tabs = [
    ...
    { href: "/settings", label: "More", icon: MoreHorizontal, isMore: true as const },
  ];
  ...
  return (
    <nav className="glass fixed inset-x-0 bottom-0 z-30 flex !rounded-none !rounded-t-glass px-2 pb-[env(safe-area-inset-bottom)] pt-1.5 lg:hidden" aria-label="Primary">
      {tabs.map(({ href, label, icon: Icon, isMore }) => {
        ...
        return (
          <Link key={label} href={href} aria-current={active ? "page" : undefined} className="relative flex flex-1 flex-col items-center gap-0.5 rounded-xl py-1.5 text-[11px] font-medium">
