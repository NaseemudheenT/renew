...
export function UploadZone({ uid }: { uid: string }) {
  ...
  async function handleFiles(files: FileList | File[]) {
    ...
    for (const file of list) {
      ...
      try {
        await uploadAndSaveDocument(uid, file, { category }, (pct) =>
          patch(id, { pct }),
        );
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div
        ...
        className={cn(
          "glass flex cursor-pointer flex-col items-center justify-center gap-2 border-2 border-dashed p-8 text-center transition-all",
          dragging
            ? "border-[var(--focus-ring)] bg-[var(--glass-bg-strong)] scale-[1.01]"
            : "border-[var(--field-border)] hover:border-[var(--focus-ring)]/60",
        )}
      >
        <motion.span
          animate={dragging ? { y: -4 } : { y: 0 }}
          className="glass grid size-14 place-items-center !rounded-full"
        >
          <UploadCloud className="size-6 text-[var(--color-gold-500)]" />
        </motion.span>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-body truncate text-sm">{it.name}</span>
