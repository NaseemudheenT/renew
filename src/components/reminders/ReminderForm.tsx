...
export function ReminderForm({
  ...
  const [time, setTime] = useState(() => toTimeInput(initial?.dueAt ?? Date.now()));
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      <Input label="Reminder" placeholder="e.g. Renew car insurance" value={title} autoFocus onChange={(e) => setTitle(e.target.value)} error={error ?? undefined} />
      <div className="grid grid-cols-2 gap-3">
        <Input label="Date" type="date" value={date} onChange={(e) => setDate(e.target.value)} />
        <div>
          <label className="mb-2 block text-sm font-medium text-[var(--text-body)]">Time</label>
          {hasTime ? (
            <Input type="time" value={time} onChange={(e) => setTime(e.target.value)} />
          ) : (
            <button type="button" onClick={() => setHasTime(true)} className="h-12 w-full rounded-2xl border border-[var(--field-border)] bg-[var(--field-bg)] text-sm text-[var(--text-muted)] transition-colors hover:text-[var(--text-strong)]">
              Add a time
            </button>
          )}
        </div>
      ...
      {hasTime && (
        <button type="button" onClick={() => setHasTime(false)} className="-mt-2 self-start text-xs text-[var(--text-muted)] hover:text-[var(--text-strong)]">
          Make it all-day
        </button>
      )}
      <div className="grid grid-cols-2 gap-3">
        ...
        <Select label="Category" value={category} onChange={(e) => setCategory(e.target.value as Category)} options={categoryOptions} />
      </div>
      ...
      <Textarea label="Notes (optional)" placeholder="Anything worth remembering…" value={notes} onChange={(e) => setNotes(e.target.value)} />
      <div className="mt-1 flex items-center justify-end gap-3">
        <AnimatedButton type="button" variant="ghost" onClick={onCancel} disabled={submitting}>Cancel</AnimatedButton>
        <AnimatedButton type="submit" loading={submitting}>{initial ? "Save changes" : "Create reminder"}</AnimatedButton>
      </div>
    </form>
  );
}
